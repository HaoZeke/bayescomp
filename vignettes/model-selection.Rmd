---
title: "Model Selection and Prior Sensitivity"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Model Selection and Prior Sensitivity}
...
Choosing the right response family and checking that results do not depend
on prior assumptions are two steps that belong in every Bayesian analysis.
This vignette covers:

1. How `bc_suggest_family()` picks a family and why its recommendations
   matter
2. Comparing candidate models with `bc_loo_compare()`
...
The true effect is `log(0.6)`, corresponding to a 40% reduction in counts
for `method_2` relative to `method_1`.  The shape parameter of 5 implies
substantial overdispersion (variance much larger than the mean).

## How bc_suggest_family() works

`bc_suggest_family()` inspects the response vector and applies the
following rules:
...
A common mistake is modelling counts or positive-only data with a Gaussian
family.  `bc_suggest_family()` warns against this:
...
To make the case empirically, fit the same data with negative binomial and
Gaussian families, then compare using LOO-CV:

```{r fit-compare, eval = FALSE}
fit_nb <- bc_fit(
  sim_count,
  response = "count",
...
indistinguishable.  The NB shape parameter reported by `bc_fit()` after
fitting tells you which regime you are in:
...
To check sensitivity, refit the model with tighter and wider priors on the
fixed effects:
...
For well-identified models with 20+ systems, all three should have similar
elpd values.

## When to use Student-t vs Gaussian

Some benchmark responses are real-valued (e.g., energy differences, barrier
errors).  These can be negative, so the log link families (NB, Gamma) do
not apply.
...
occasional large error that is common in computational chemistry benchmarks
(e.g., a method failing on one system and producing a wildly wrong energy).
The degrees-of-freedom parameter (`nu`) is estimated from the data; values
near 3-5 indicate heavy tails, while values above 30 approach Gaussian
behaviour.

```{r fit-real, eval = FALSE}
fit_student <- bc_fit(
  sim_real,
  response = "value",
...
1. Call `bc_suggest_family()` on your response vector to get the
   recommended family
2. Fit the recommended model with `bc_fit()`
3. If you suspect an alternative family might work, fit it too
4. Compare with `bc_loo_compare()` -- prefer the model with highest elpd
5. Refit with `bc_default_priors()` at different scales (0.5, 1, 2) for
   the fixed-effect SD
